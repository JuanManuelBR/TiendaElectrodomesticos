  ...
  methods: {
    async cargarMarcas() {
      try {
        const response = await fetch(
          'http://localhost:8090/electrodomestico/control/marcas',
          {
            method: 'GET',
            headers: {
    ...
    async filtrar() {
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:8090/electrodomestico/control/filtrar/${marcaSeleccionada}`,
          {
            method: 'GET',
            headers: {
              'Authorization': 'Basic ' + btoa('admin:admin')
        ...
        if (response.ok && response.headers.get('content-length') !== '0') {
          ...
          if (responseText && responseText.trim() !== '') {
            ...
            this.controles = data.map(control => {
              ...
              // Código TV (TryGetProperty televisor + ValueKind == Object)
              let codigoTv = '';
              if (control.televisor !== null && 
                  control.televisor !== undefined && 
                  typeof control.televisor === 'object') {
                if (control.televisor.codigo !== null && 
                    control.televisor.codigo !== undefined && 
                    typeof control.televisor.codigo === 'number') {
                  codigoTv = control.televisor.codigo.toString();
                }
              }
