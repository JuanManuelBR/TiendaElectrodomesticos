  ...
  <div class="container">
    <div class="form-card">
      ...
      <div class="search-section">
        <label class="form-label">Código Control Eliminar:</label>
        <input 
          type="number" 
...
export default {
  ...
  methods: {
    async buscar() {
      try {
        // Parsear código igual que en C#
        const codigoBuscar = parseInt(this.codigoBuscar);
        
        if (isNaN(codigoBuscar)) {
          this.mostrarMensaje('Por favor ingrese un código válido', 'warning');
          return;
        }
        ...
        if (response.ok && response.headers.get('content-length') !== '0') {
          ...
          if (responseText && responseText.trim() !== '') {
            ...
            this.limpiarCampos();
            this.mostrarMensaje('No se encontró el control con ese código', 'warning');
          }
        } else {
          this.limpiarCampos();
          this.mostrarMensaje('No se encontró el control con ese código', 'warning');
    ...
    async eliminar() {
      ...
      try {
        const codigoEliminar = parseInt(this.codigoBuscar);

        const response = await fetch(
          `http://localhost:8090/electrodomestico/control/${codigoEliminar}`,
          {
            method: 'DELETE',
            headers: {
              'Authorization': 'Basic ' + btoa('admin:admin')
