  ...
  methods: {
    async listar() {
      try {
        ...
        const response = await fetch('http://localhost:8090/electrodomestico/control', {
          method: 'GET',
          headers: {
        ...
        if (response.ok) {
          ...
          this.controles = data.map(control => {
            ...
            const alcance = control.alcance ?? control.distancia ?? control.precio ?? '';
            const fechaVenta = this.formatFecha(control.fechaVenta ?? control.fecha ?? '');

            return { codigo, nombre, marca, alcance, fechaVenta };
          });
...
<style scoped>
/* 🔹 Conservado exactamente como tu diseño original */
.container {
  min-height: 100vh;
  display: flex;
