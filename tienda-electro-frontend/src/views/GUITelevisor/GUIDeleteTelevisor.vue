  ...
  <div class="container">
    <div class="form-card">
      ...
      <div class="search-section">
        <label class="form-label">Código Televisor Eliminar:</label>
        <input 
          type="number" 
          v-model="codigoBuscar" 
          class="form-input"
          placeholder="Ingrese código a eliminar"
          @keyup.enter="buscar"
        />
      </div>
  ...
  methods: {
    async buscar() {
      try {
        ...
        const codigoBuscarInt = parseInt(this.codigoBuscar);

        const response = await fetch(
          `http://localhost:8090/electrodomestico/${codigoBuscarInt}`,
          {
            method: 'GET',
            headers: {
        ...
        if (response.ok) {
          ...
          this.televisor.precio = data.precio ? data.precio.toString() : '';
          ...
          // Manejo robusto del control remoto (igual que en C#)
          if (data.controlRemoto) {
            if (typeof data.controlRemoto === 'object' && data.controlRemoto.codigo) {
              this.televisor.codigoControl = data.controlRemoto.codigo.toString();
            } else if (typeof data.controlRemoto === 'number') {
              this.televisor.codigoControl = data.controlRemoto.toString();
            } else {
    ...
    async eliminar() {
      ...
      try {
        const codigoEliminarInt = parseInt(this.codigoBuscar);

        const response = await fetch(
          `http://localhost:8090/electrodomestico/${codigoEliminarInt}`,
          {
            method: 'DELETE',
            headers: {
              'Authorization': 'Basic ' + btoa('admin:admin')
...
.form-card {
  ...
  width: 150%;         /* ocupa todo el ancho disponible del contenedor */
  max-width: 800px;     /* pero no supera los 900px */
  margin: 2 auto;       /* centra el elemento horizontalmente */
