      ...
      try {
        ...
        if (response.ok) {
          ...
          this.televisores = data.map(televisor => {
            ...
            let codigoControl = '';
            if (televisor.controlRemoto !== null && televisor.controlRemoto !== undefined) {
              // Si es un objeto con propiedad codigo
              if (typeof televisor.controlRemoto === 'object' && televisor.controlRemoto.codigo !== undefined) {
                codigoControl = televisor.controlRemoto.codigo.toString();
              }
              // Si es un número directo
              else if (typeof televisor.controlRemoto === 'number') {
                codigoControl = televisor.controlRemoto.toString();
              }
            }
...
.btn-exit:hover {
  background: #cc6666;
  transform: translateY(-2px);
  box-shadow: 0 5px 15px rgba(255, 128, 128, 0.4);
}
