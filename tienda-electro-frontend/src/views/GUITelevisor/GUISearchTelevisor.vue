    ...
    async buscar() {
      try {
        ...
        const response = await fetch(
          `http://localhost:8090/electrodomestico/${codigoBuscar}`,
          {
            method: 'GET',
            headers: {
        ...
        if (response.ok) {
          ...
          if (data.controlRemoto !== null && 
              data.controlRemoto !== undefined && 
              typeof data.controlRemoto === 'object') {
            // Es un objeto, buscar la propiedad codigo
            if (data.controlRemoto.codigo !== undefined) {
              this.televisor.codigoControl = data.controlRemoto.codigo.toString();
            } else {
              this.televisor.codigoControl = 'No hay control asignado';
            }
          } else {
            // Es null, undefined o no es un objeto
            this.televisor.codigoControl = 'No hay control asignado';
          }
        } else {
