    ...
    <div class="list-card">
      ...
      <div class="table-container">
        ...
        <div v-else class="empty-state">
          ...
          <p class="hint">Seleccione una marca y haz clic en "Filtrar"</p>
        </div>
      </div>
...
export default {
  ...
  methods: {
    async cargarMarcas() {
      try {
        const response = await fetch('http://localhost:8090/electrodomestico/marcasTelevisores', {
          method: 'GET',
          headers: {
    ...
    async filtrar() {
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:8090/electrodomestico/filtrarTelevisores/${this.marcaSeleccionada}`,
          {
            method: 'GET',
            headers: {
              'Authorization': 'Basic ' + btoa('admin:admin')
        ...
        if (response.ok) {
          ...
          this.televisores = data.map(tv => {
            const codigo = tv.codigo;
            const nombre = tv.nombre;
            const precio = tv.precio;
            const marca = tv.marca;
            const fecha = this.formatFecha(tv.fechaCreacion);
            
            // Manejar controlRemoto igual que en C#
            let codigoControl = '';
            if (tv.controlRemoto !== null && tv.controlRemoto !== undefined) {
              // Si es un objeto con propiedad codigo
              if (typeof tv.controlRemoto === 'object' && tv.controlRemoto.codigo !== undefined) {
                codigoControl = tv.controlRemoto.codigo.toString();
              }
              // Si es un número directo
              else if (typeof tv.controlRemoto === 'number') {
                codigoControl = tv.controlRemoto.toString();
              }
            }
