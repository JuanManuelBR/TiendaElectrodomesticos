...
namespace POCClienteEmpleado.com.mycompany.tiendaelectrodomestico.view
{
    public partial class GUIFiltrarMarcaControl : Form
    {
        ...
        private void GUIFiltrarControl_Load(object sender, EventArgs e)
        {
            ...
            gridControles.Columns.Add("Codigo", "Código");
            ...
            try
            {
                var request = new RestRequest("/electrodomestico/control/marcas", Method.Get);
        ...
        private void btnFiltrar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var request = new RestRequest($"/electrodomestico/control/filtrar/{marcaSeleccionada}", Method.Get);
                var response = client.Execute(request);
                ...
                if (response.IsSuccessful && !string.IsNullOrWhiteSpace(response.Content))
                {
                    ...
                    using (JsonDocument doc = JsonDocument.Parse(response.Content))
                    {
                        ...
                        {
                            int codigo = control.GetProperty("codigo").GetInt32();
