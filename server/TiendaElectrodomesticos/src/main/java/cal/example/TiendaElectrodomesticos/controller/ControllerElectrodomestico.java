...
public class ControllerElectrodomestico {
    ...
    @DeleteMapping("/{codigoTelevisor}")
    public ResponseEntity<?> eliminarTelevisor(@PathVariable int codigoTelevisor) {
        try {
            boolean eliminado = serviceElectrodomestico.deleteElectrodomestico(codigoTelevisor);

            if (eliminado) {
                return ResponseEntity.ok("Televisor eliminado correctamente. El control remoto fue desasignado.");
            } else {
    ...
    public ResponseEntity<?> eliminarControl(@PathVariable int codigo) {
        try {
            ...
            if (eliminado) {
                ...
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body("No se encontró el control remoto con ese código.");
            }
        } catch (Exception ex) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ex.getMessage());
        }
