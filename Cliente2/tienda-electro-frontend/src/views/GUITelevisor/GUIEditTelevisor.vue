      ...
      <div class="form-grid">
        <div class="form-left">
          ...
          <div class="hint-text" v-if="televisorCargado">
            💡 Ingrese los códigos de controles separados por comas. Puede agregar o quitar controles.
          </div>
        </div>
  ...
  data() {
    return {
      ...
      televisor: {
        ...
        marca: '',
        fechaCreacion: '',
        codigosControles: '' // String con códigos separados por comas
      },
      controlesActuales: [], // Array de números
  ...
  methods: {
    async buscar() {
      try {
        ...
        if (response.ok) {
          ...
          this.televisor.codigosControles = this.controlesActuales.join(',');
    ...
    async actualizar() {
      try {
        ...
        const fechaCreacion = new Date(this.televisor.fechaCreacion);

        // 1. Actualizar datos del televisor (sin controles en el body)
        const body = {
          tipo: 'televisor',
        ...
        // 2. Manejo de asignación/desasignación de controles
        const nuevosCodigos = [];
        ...
        const agregar = nuevosCodigos.filter(c => !this.controlesActuales.includes(c));
        const quitar = this.controlesActuales.filter(c => !nuevosCodigos.includes(c));

        let advertencias = [];

        // 3. Asignar nuevos controles
        for (const codigoControl of agregar) {
          ...
          if (!responseCheck.ok) {
            advertencias.push(`El control ${codigoControl} no existe. No se asignará.`);
            continue;
          }
          ...
          if (!responseAsignar.ok) {
            advertencias.push(`No se pudo asignar el control ${codigoControl}. Es posible que ya esté asignado a otro televisor.`);
          }
        }

        // 4. Desasignar controles eliminados
        for (const codigoControl of quitar) {
          ...
          if (!responseDesasignar.ok) {
            advertencias.push(`No se pudo desasignar el control ${codigoControl}.`);
          }
        }

        // Mostrar resultado
        if (advertencias.length > 0) {
          this.mostrarMensaje('Televisor actualizado con advertencias:\n' + advertencias.join('\n'), 'warning');
        } else {
          this.mostrarMensaje('Televisor actualizado correctamente', 'success');
        }

        // Actualizar la lista actual de controles para futuras ediciones
